...
class Recoder:
    ...
    def combine(self):
        ...
        wf2 = wave.open('resources/Sound/introduce.wav','rb')
        data2 = wf2.readframes(self.SAMPLING_RATE*self.NUM_SAMPLES)

        VoiceString = str(data) + str(data2)

        wf3 = wave.open('combine.wav', 'wb')
        wf3.setnchannels(1)
        wf3.setsampwidth(2)
        wf3.setframerate(self.SAMPLING_RATE)
        wf3.writeframes(data2+data)
        wf3.close()
