...
class FaceRecognition():
    ...
    #     logo = cv.imread('resources/logo/logo.jpg')
    #     t_end = time.time() + sec
    #     capturedNum = 0
    #     #删掉之前的照片
    #     self.delByName(name)
...
#     def trainModel(self):
#         #{'hai':[url1,url2]
#         self.train_faces = self.search_faces('resources/image/img/')
...
#方法3
    def trainModel(self):
        ...
        for subdirs in ['chen','lin','luo','sun','wu','zhang']:    
            #sun lin chen
...
#     def __init__(self):
#         # 哈尔级联人脸定位器
#         self.fd = cv.CascadeClassifier('resources/ml_data/haar/face.xml')
#         self.ed = cv.CascadeClassifier('resources/ml_data/haar/eye.xml')
#         self.nd = cv.CascadeClassifier('resources/ml_data/haar/nose.xml')
#         self.savePath = 'resources/image/img/'
#         self.train_faces = self.search_faces('resources/image/img/')
#
#     def delByName(self,name):
